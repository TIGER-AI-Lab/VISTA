...
from models.idefics2 import Idefics2ForConditionalGeneration, Idefics2Processor
...
class Idefics2():
    def __init__(self, model_path="HuggingFaceM4/idefics2-8b", device="cuda") -> None:
        """Llava model wrapper
        ...
        model = Idefics2ForConditionalGeneration.from_pretrained(model_path, torch_dtype=torch.bfloat16, _attn_implementation=attn_implementation)
        self.model = model.to(device).eval()
        self.processor = Idefics2Processor.from_pretrained(model_path, do_image_splitting=False)
        self.patch_size = self.model.config.vision_config.patch_size
        self.conv = conv_templates["idefics_2"]
...
if __name__ == "__main__":
    model = Idefics2(model_path="/cpfs/data/user/weiming/checkpoints/TIGER-Lab/Mantis-8B-Idefics2")
    print("############################################################################################")
    test_input = [
        {
            "type": "video",
    ...
    test_input = [
        ...
        {
            "type": "text",
            "content": "<image> describe the video in detail."
        }
    ]
    print(model(test_input))
    print("############################################################################################")
