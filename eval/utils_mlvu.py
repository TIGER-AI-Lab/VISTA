...
class MLVUDataset(Dataset):
    def __init__(self, dataset_path: str, sample_config: dict):
        ...
        self.data_path = osp.join(dataset_path, "video")
        ...
        for subset in json_list:
            ...
            self.full_data_list.extend(data_list)

    def __len__(self):
        return len(self.full_data_list)

    def __getitem__(self, idx) -> dict:
        subset = self.full_data_list[idx]['subset']
        frames, durations = load_decord(
            src_path=osp.join(self.data_path, subset, self.full_data_list[idx]['video']),
            **self.sample_config
        )
        ...
        for i in range(len(item['candidates'])):
            if item['candidates'][i] == item['answer']:
                item['answer'] = chr(65+i)
                break
