...
class HighresQADataset(Dataset):
    def __init__(self, dataset_path: str, json_path, sample_config: dict):
        ...
        with open(json_path, "r") as f:
            self.full_data_list = [json.loads(line) for line in f]

    def __len__(self):
        return len(self.full_data_list)

    def __getitem__(self, idx) -> dict:
        src_path = osp.join(self.data_path, self.full_data_list[idx]['path'])
        ...
        text = '\n'.join([
            ...
            item['qa']['question']
        ] + [f"{item['mcq']['options'][i]}" for i in range(len(item['mcq']['options']))])

        item["answer_text"] = copy.deepcopy(item["mcq"]["correct_option"])
        item["answer"] = item["mcq"]["correct_option"][0]
