...
class LongVideoBenchDataset(Dataset):
    def __init__(self, dataset_path: str, sample_config: dict):
        super().__init__()
        json_path = osp.join(dataset_path, "lvb_val.json")
        
        self.data_path = osp.join(dataset_path, "videos")
    ...
    def __getitem__(self, idx) -> dict:
        frames, durations = load_decord(
            src_path=osp.join(self.data_path, self.full_data_list[idx]['video_path']),
            **self.sample_config
        )
        ...
        item['answer'] = chr(65 + item['correct_choice'])
