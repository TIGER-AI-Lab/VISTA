...
class Conversation:
    ...
    def get_prompt(self):
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            ret = self.sep2 if self.system == "" else self.sep2 + self.system + self.sep
            for i, (role, message) in enumerate(messages):
                if message:
                    if type(message) is tuple:
                        message, _, _ = message
            ...
            for i, (role, message) in enumerate(messages):
                if message:
                    ...
                    ret += role
        elif self.sep_style == SeparatorStyle.LLAMA_3 or self.sep_style == SeparatorStyle.MINICPM or self.sep_style == SeparatorStyle.KANGAROO:
            # llama3 and minicpm has a bos token
            ret = self.sep2 if self.system == "" else self.sep2 + self.system + self.sep
            for role, message in messages:
                if message:
                    if type(message) is tuple:
            ...
            for role, message in messages:
                if message:
                    ...
                    ret += role + ":"
        elif self.sep_style == SeparatorStyle.PHI3_V:
            ret = "" if self.system == "" else self.system + self.sep + "\n"
            for i, (role, message) in enumerate(messages):
                if message:
                    if type(message) is tuple:
...
conv_minicpm_v = Conversation(
    system="",
    roles=("user", "assistant"),
    messages=(),
    offset=0,
    sep_style=SeparatorStyle.MINICPM,
    sep="<|eot_id|>",
    sep2="<|begin_of_text|>",
)

conv_falcon_mamba = Conversation(
    system="",
    roles=("<|im_start|>user\n", "<|im_start|>assistant\n"),
    messages=(),
    offset=0,
...
conv_kangaroo = Conversation(
    system="<|start_header_id|>system<|end_header_id|>\n\nYou are a helpful assistant, Review the given video and answer the question associated with its visual elements. 你是一个乐于助人的助手，审阅所提供的视频，并回答与其视觉元素相关的问题。",
    roles=("user", "assistant"),
    messages=(),
    offset=0,
    sep_style=SeparatorStyle.KANGAROO,
    sep="<|eot_id|>",
    sep2="<|begin_of_text|>",
)

conv_phi3_v = Conversation(
    system="",
    roles=("<|user|>\n", "<|assistant|>\n"),
    messages=(),
    offset=0,
    sep_style=SeparatorStyle.PHI3_V,
    sep="<|end|>",
)
...
conv_templates = {
    ...
    "minicpm": conv_minicpm_v,
    "falcon_mamba": conv_falcon_mamba,
    "qwen2": conv_qwen2,
    "kangaroo": conv_kangaroo,
    "phi3_v": conv_phi3_v,
}
